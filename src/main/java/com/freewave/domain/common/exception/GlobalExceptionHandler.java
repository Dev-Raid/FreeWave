...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ExceptionResponse> handleInvalidRequest(RuntimeException e, HttpServletRequest request) {
    ...
    public ResponseEntity<ExceptionResponse> handleNotFound(RuntimeException e, HttpServletRequest request) {
        return ResponseEntity
                .status(HttpStatus.NOT_FOUND)
                .body(new ExceptionResponse(HttpStatus.NOT_FOUND, e.getMessage(), URI.create(request.getRequestURI())));
    }

    // 415 Unsupported Media Type (지원되지 않는 미디어 유형)
    @ExceptionHandler(UnsupportedFileTypeException.class)
    @ResponseStatus(HttpStatus.UNSUPPORTED_MEDIA_TYPE)
    public ResponseEntity<ExceptionResponse> handleUnsupportedMediaType(UnsupportedFileTypeException e, HttpServletRequest request) {
        return ResponseEntity
                .status(HttpStatus.UNSUPPORTED_MEDIA_TYPE)
                .body(new ExceptionResponse(HttpStatus.UNSUPPORTED_MEDIA_TYPE, e.getMessage(), URI.create(request.getRequestURI())));
    }

    // 422 Unprocessable Entity (처리할 수 없는 엔티티)
    @ExceptionHandler(FileProcessingException.class)
    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    public ResponseEntity<ExceptionResponse> handleUnprocessableEntity(FileProcessingException e, HttpServletRequest request) {
        return ResponseEntity
                .status(HttpStatus.UNPROCESSABLE_ENTITY)
                .body(new ExceptionResponse(HttpStatus.UNPROCESSABLE_ENTITY, e.getMessage(), URI.create(request.getRequestURI())));
    }
